...
def post_list_phone(new_phone_list: List[PhoneModel]):
    ...
    return Response(content=json.dumps({"message": "Created successfully"}),media_type="application/json",status_code=201)
...
def get_phone_by_id(id : str):
    ...
    return Response(content=json.dumps({"error":f"Phone id: {id} does not exist or not found"}),media_type="application/json",status_code=404)
